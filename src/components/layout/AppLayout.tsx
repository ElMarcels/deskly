...
export default function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a1a] text-[#e0e0ff]">
      ...
      <aside
        className={`fixed left-0 top-0 z-40 flex h-full w-64 flex-col bg-[#12122a]/70 border-r border-[rgba(168,85,247,0.15)] backdrop-blur-xl transition-transform duration-300 ease-in-out ${
      ...
      >
        <div className="flex items-center gap-3 border-b border-[rgba(168,85,247,0.15)] px-6 py-5">
          ...
          <span className="text-xl font-bold tracking-tight gradient-neon">
            Deskly
          </span>
        </div>
            ...
            return (
              <Link
                ...
                className={`group flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition-all duration-200 ${
                  isActive
                    ? "bg-[rgba(168,85,247,0.15)] text-[#a855f7] border border-[rgba(168,85,247,0.3)] shadow-[0_0_15px_rgba(168,85,247,0.15)]"
                    : "text-[#e0e0ff]/50 hover:bg-[#1a1a3e]/50 hover:text-[#e0e0ff]/80 border border-transparent"
                }`}
              >
                <Icon
                  size={18}
                  className={`flex-shrink-0 transition-colors ${
                    isActive ? "text-[#a855f7] drop-shadow-[0_0_6px_rgba(168,85,247,0.5)]" : "text-[#e0e0ff]/30 group-hover:text-[#e0e0ff]/60"
                  }`}
                />
                <span>{link.label}</span>
                {isActive && (
                  <div className="ml-auto h-1.5 w-1.5 rounded-full bg-[#a855f7] shadow-[0_0_8px_rgba(168,85,247,0.7)]" />
                )}
              </Link>
            );
          })}
        ...
        <div className="border-t border-[rgba(168,85,247,0.15)] px-4 py-4">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-[#e0e0ff] truncate">Estudiante</p>
              <p className="text-xs text-[#e0e0ff]/40 truncate">estudiante@deskly.app</p>
            </div>
            <button className="rounded-lg p-2 text-[#e0e0ff]/30 hover:bg-red-500/10 hover:text-red-400 transition-colors cursor-pointer">
              <LogOut size={16} />
            </button>
          </div>
        </div>
